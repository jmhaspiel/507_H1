...
class Deck(object):
    ...
    def __str__(self):
        ...
        return "\n".join(total)  # returns a multi-line

    def pop_card(self, i=-1):
        return self.cards.pop(i)  # Taken off
    ...
    def replace_card(self, card):
        ...
        for c in self.cards:  # each card in self.cards (the initial list)
            card_strs.append(c.__str__())  # to the empty list
...
def show_song(inp="Winner"):  #
    slist = ["win", "winner", "hurrah", "hooray"]
    songs_resp = helper_functions.get_and_cache_songs(random.choice(slist))
